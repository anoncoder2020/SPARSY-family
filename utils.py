...
def remove_edges(adj, top_k, pruned_ratio):
    row_sum_init = adj.sum(axis=1)
    rowsum = np.squeeze(np.array(row_sum_init))
    idx = np.argpartition(rowsum, -top_k)[-top_k:]
    total_remove_edges = 0
    for i in range(len(idx)):
        row_sum_init = adj.sum(axis=1)
        rowsum = np.squeeze(np.array(row_sum_init))
        node_id = idx[i]
        num_edge_remove = int(round(rowsum[node_id] * pruned_ratio))
        total_remove_edges += num_edge_remove
        if num_edge_remove > 0:
            col_idx = adj[node_id].indices
            remove_id = np.random.choice(col_idx, num_edge_remove, replace=False)
            adj[node_id, remove_id] = 0
            adj[remove_id, node_id] = 0
            adj.eliminate_zeros()
    return adj.tocsr(), total_remove_edges
...
def batch_cluster(y, test_idx_range, labels, chosen_cluster, clusters, adj, features, degree_top_k_node, percent_edges, percent_trained_node):
    ...
    rm_nodes = int(p_adj.shape[0] * degree_top_k_node)
    if rm_nodes > 0:
        p_adj, num_remove = remove_edges(p_adj, top_k=int(p_adj.shape[0] * degree_top_k_node),
...
def minbatch(labels, y, test_idx_range, block_size, num_clusters, set_seed, clusters, adj, features, degree_top_k_node, percent_edges, percent_trained_node):
    whole_dict = {}
    ...
    for idx in range(0, num_clusters, block_size):
        ...
        chosen_cluster, idx_train, idx_val, idx_test = batch_cluster(y, test_idx_range, labels, cluster_idx,
                                        clusters, adj, features, degree_top_k_node, percent_edges, percent_trained_node)
